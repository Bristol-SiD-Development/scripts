# Modified zangle script that plots the transverse momentum of particles which left the detector (didn't hit)
...
def get_pos(event): # Each detector is a 'collection', the No. of Elements are the hits.
	...
	m = TH1F("m", "Transverse Momenta of Particles that Left Detector", 100, 0, 2) # creates the histogram with ("name,"description", nbins, x low, x high)
...
def output(outputDirectory, outputName, inputFile):
	...
	if os.path.isdir(outputDirectory) and outputName == 'default':
		...
		outputName = os.path.basename(inputpath) + '_angle_Tmoms.png' # edit this to change the tag on the endof the image
