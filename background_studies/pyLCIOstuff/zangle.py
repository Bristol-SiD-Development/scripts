# Modified posPlot script that plots Zenith angle of electrons and positrons.
...
from ROOT import TFile, TNtuple, TCanvas, TH1F, TAxis, TSystem, TGraph, TGraph2D, TTree, TH3F, TMath # not all are used but I'm being safe
...
def get_pos(event): # Each detector is a 'collection', the No. of Elements are the hits.
	...
	for ding in mcpart: # for every entry in the collection - can be called whatever
		...
		if ding.getPDG() == 11 or ding.getPDG() == -11 and mom[2]!=0: # condition that only allows electrons or positrons (their pdg id) also, the momenta cant be 0
			...
			if t < 0.0075: 	# this is the cut value of 7.5 GeV
				...
				n.Fill(theta, 1) # fills up the histogram so the value of theta gets 1 extra weight per cycle
...
def output(outputDirectory, outputName, inputFile):
	...
	if os.path.isdir(outputDirectory) and outputName == 'default':
		...
		outputName = os.path.basename(inputpath) + '_angle_compare.png' # change this entry to tag stuff on the end of the image name
...
def main():
	...
	for filename in inputFiles:
		...
		for event in reader:
			...
			n, m = get_pos(event) # assigns the output histograms of the funtion to the variables n and m
