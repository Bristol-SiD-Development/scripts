# Modified posPlot script to create a histogram of hits in the x-y plane of the Beam calorimeter.
...
def draw_hist(BCALhist, output):
	...
	img.WriteImage(output)	# Saves it as png file with this name the the input file directory.
...
def output(outputDirectory, outputName, inputFile):
	...
	if os.path.isdir(outputDirectory) and outputName == 'default':
		...
		outputName = os.path.basename(inputpath) + '_beamcal.png' # cahnge this to edit the tag on the end of image file
...
def main():
	...
	for filename in inputFiles:
		...
		for event in reader:
			...
			draw_hist(hist, outputFile)
				
			break # there's only one event - this is a cheap fix to stop it replacing histogram with a blank one
